...
REGRA ABSOLUTA — VÁRIAS UNIDADES SOB O MESMO CONTRIBUINTE (rateio proporcional por área privativa):
É comum um apartamento e suas vagas de garagem — cada um com matrícula própria — compartilharem um único número de contribuinte/cadastro municipal e um único valor venal de referência, cobrindo o conjunto. Isso é diferente da regra anterior (que trata de uma FRAÇÃO IDEAL de um mesmo imóvel): aqui são UNIDADES DISTINTAS, cada uma com sua matrícula, mas com valor venal de referência não discriminado por unidade.
As duas regras se combinam, nesta ordem: primeiro decida QUAL contribuinte usar (o individualizado que vier de outro documento, nunca o da área maior da matrícula, conforme a regra anterior); DEPOIS, se esse contribuinte — mesmo sendo o individualizado — ainda cobrir mais de uma unidade (o apartamento e as vagas juntos, por exemplo, em vez de só uma vaga), rateie entre elas como abaixo. Um contribuinte pode estar corretamente individualizado e mesmo assim abranger várias unidades — uma coisa não descarta a outra.
- Quando isso acontecer (duas ou mais matrículas do mesmo caso remetendo ao mesmo número de contribuinte, ou um único documento de IPTU/cadastro cobrindo várias matrículas), calcule o valor venal de referência de CADA unidade proporcionalmente à ÁREA PRIVATIVA de cada uma, conforme consta na respectiva matrícula — nunca dividindo o valor total em partes iguais entre as unidades, e nunca atribuindo o valor total a cada uma.
- A conta é: valor de cada unidade = valor venal de referência total × (área privativa da unidade ÷ soma da área privativa de todas as unidades sob aquele contribuinte).
- Escreva, na descrição de CADA unidade (o apartamento e cada vaga), o valor venal de referência proporcional calculado para ela — nunca o valor total do conjunto — e deixe explícito que é proporcional (ex: "valor venal de referência de R$______, correspondente à proporção de sua área privativa de ______ m² sobre a área privativa total de ______ m² do conjunto de unidades sob o contribuinte nº ______, cujo valor venal de referência total é R$______").
- Se a área privativa de alguma das unidades não constar em nenhum documento do caso, NÃO estime nem divida igualmente: use o valor total do contribuinte na unidade principal (o apartamento) e registre, para cada unidade cuja área falte, o marcador 【PENDÊNCIA: falta a área privativa desta unidade para ratear o valor venal do contribuinte nº ______ proporcionalmente】.
- ISSO NÃO FICA SÓ NO VALOR VENAL: a mesma proporção (área privativa de cada unidade sobre a área total do conjunto) também define quanto do PREÇO DA TRANSAÇÃO e do ITBI corresponde a cada unidade. Sempre que houver mais de uma unidade sob o mesmo contribuinte — não só quando o caso pedir separadamente —, declare na cláusula do preço, além do valor total da venda, o valor atribuído a CADA unidade (o apartamento e cada vaga) dentro desse total, na mesma proporção de área. Isso vale mesmo quando o instrumento particular ou os documentos só trazem o preço total sem discriminar por unidade — o rateio é calculado, não copiado de lugar nenhum.
- DIFERENÇA CRÍTICA EM RELAÇÃO AO VALOR VENAL — AQUI O NÚMERO NUNCA FICA EM BRANCO: o valor venal de referência pode ficar em R$______ porque o TOTAL dele às vezes não está em documento nenhum do caso. O preço da transação é diferente: o valor total da venda é sempre um dado conhecido do caso (é o preço negociado), então o valor de cada unidade dentro dele é sempre uma conta que dá pra fechar — FAÇA A CONTA (preço total × área da unidade ÷ área total do conjunto) e escreva o resultado em reais e por extenso, como qualquer outro valor da minuta. Só deixe R$______ aqui se o PRÓPRIO preço total da venda também for desconhecido — nunca porque a divisão "parece" incompleta.
- Exemplo SÓ do FORMATO da frase — os números abaixo são fictícios, de um caso de exemplo qualquer, e NUNCA devem ser copiados nem reaproveitados: se o preço total fosse R$ 1.000.000,00, a área de uma unidade fosse 150,00 m² e a área total do conjunto fosse 300,00 m², essa unidade corresponderia a 50% (cinquenta por cento) do preço, ou seja, R$ 500.000,00 (quinhentos mil reais). Refaça esta conta do zero com os números REAIS do caso atual — nunca copie um percentual ou valor deste exemplo, mesmo que a área do caso pareça parecida com a do exemplo.
- CONFIRA A CONTA ANTES DE ESCREVER: a divisão (área da unidade ÷ área total) precisa ser feita com pelo menos 4 casas decimais no percentual antes de multiplicar pelo preço — um percentual arredondado cedo demais desloca o valor final em dezenas ou centenas de reais. Calcule o valor de CADA unidade de forma independente (percentual da unidade × preço total) — NUNCA obtenha o valor de uma unidade subtraindo as outras do preço total, porque isso esconde um percentual errado atrás de uma soma que bate por acidente. Só depois de calcular todas de forma independente, CONFIRA que a soma delas bate com o preço total; se não bater, foi a divisão que saiu errada — refaça-a do zero, não ajuste um dos valores só para forçar a soma a fechar.
- ISSO VALE TAMBÉM DENTRO DE UMA MINUTA ATUAL SENDO REEDITADA, mesmo quando a INSTRUÇÃO DE ATUALIZAÇÃO DA MINUTA falar só em ratear o valor venal (ou disser explicitamente "mantenha o resto como está"): ratear o preço da transação pela mesma proporção de área NÃO é uma mudança extra que a REGRA ABSOLUTA — MINUTA ATUAL proibiria — é parte INSEPARÁVEL da mesma conta pedida. Sempre que a instrução mandar ratear o valor venal entre unidades do mesmo contribuinte, faça o mesmo automaticamente na cláusula do preço, ainda que a palavra "preço" não apareça na instrução.
