...
module.exports = async (req, res) => {
  ...
  const { nome, tipo, obs, dataHora, descricao } = dados;
  if (!dataHora) return res.status(400).json({ ok: false, erro: "Data e hora não informadas" });

  try {
    const driveResp = await httpPost(DRIVE_URL, {
      acao: "criar-evento-agenda",
      nome: nome || "Caso",
      tipo: tipo || "",
      obs: obs || "",
      dataHora,
    ...
    return res.status(200).json({ ok: true, eventId: driveResp.eventId || null });
  } catch (err) {
